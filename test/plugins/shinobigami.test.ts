...
test("FT", () => {
  ...
  expect(res).toStrictEqual({
    total: 4,
    mainMassage: "油断した！　術の制御に失敗し、好きな【生命力】を１点失う。",
    status: Status.Unknown,
    process: [
      "ファンブル表",
      "4[4]",
...
test("ET", () => {
  ...
  expect(res).toStrictEqual({
    total: 1,
    mainMassage: "共感（プラス）／不信（マイナス）",
    status: Status.Unknown,
    process: ["感情表", "1[1]", "共感（プラス）／不信（マイナス）"],
    isSecret: false,
    dice: [{ faces: 6, value: 1 }]
  });
});
